...
void Truncation(uni10::Matrix<double>& En, uni10::Matrix<double>& state, const int chi, int& m, bool& info)
{
    ...
    while (gap[idx] <= pow(10, -12)) // If chi at degenerate state, choose chi - 1
    {
        idx--;
        m--;
        if (idx < 0)
...
void tSDRG(vector<MPO>& MPO_chain, vector<double>& Q, vector<uni10::UniTensor<double> >& VTs, vector<int>& Vs_loc, const int chi, string dis, const int Pdis, const int Jseed, bool save_RG_info, bool& info)
{
    ...
    //cout << coeff << endl; coeff = 2 in S = 1 case
    ...
    while(MPO_chain.size() > 2)
    {
        ...
        // cout << "s1 : " << s1 << endl;
        // printf("Q[%d] : %f\n", s1,Q[s1]);
        // printf("Q[%d], Q[%d], Q[%d] : %f, %f, %f\n", s1,s1+1,s1+2,Q[s1],Q[s1+1],Q[s1+2]);
        // cout << "Q : ";
        // for(int i=0; i<Q.size(); i++)
        //     if(i == s1)
        //         cout << " | " << Q[i] << " | " << ",";
        ...
        /// RG coupling into merge list, notice that edge mpo is OBC or PBC
        if (MPO_chain.size() != 2)
        {
            if (BC == "PBC")
            {
            ...
            else if (BC == "OBC")
            {
                if (s1 != 0)
                {
                    ...
                    H12.PutTensor("H2", H2);
                    H12.Launch(H);
                    uni10::EigH(H.GetBlock(), En, state, uni10::INPLACE);   // in mkl use uni10::EigHLazy()
                    Q[s1-1] = find_highest_gap(En, chi, info);
                }
                if (s1 != Q.size()-1)
                {
